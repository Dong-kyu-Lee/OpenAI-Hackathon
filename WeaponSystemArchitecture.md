...
| `PooledProjectile.cs` | 이동, 수명, 직접/폭발 피해, 풀 상태 초기화 및 반환 |
...
- 풀은 지연 생성 방식이므로 첫 발사 순간 Instantiate가 발생할 수 있다. 필요하면 ObjectPoolManager 내부에 프리워밍을 추가한다.
- 로켓은 범위 피해 판정만 구현되어 있으며 폭발 VFX/사운드는 별도다. 추가할 경우 이펙트도 풀링한다.
