...
| # | 금지 | 이유 |
|---|---|---|
| 1 | `.unity`, `.prefab`, `.asset`, `.controller`, `.anim` 파일을 텍스트로 직접 생성/편집 | YAML + fileID/GUID 구조가 손상되면 씬 복구가 불가능하다 |
...
| 9 | 요청 범위 밖의 기능을 "있으면 좋을 것 같아서" 추가 | 8장 참조 |
...
1. **씬 파일을 직접 만들거나 편집하지 않는다.** 씬 구성이 필요하면 "어떤 씬에 무엇을 배치해야 하는지" 지시만 출력한다 (6.3 참조).
...
- 커밋을 요청받으면 형식은 다음과 같이 한다.
  ```
  feat(player): 대시 이동 추가
  fix(weapon): 총알 풀 반환 누락 수정
  refactor(ui): HUD 이벤트 채널 구독으로 변경
  chore(asset): 적 스프라이트 추가
  ```
- **코드 커밋과 씬/프리팹 커밋을 한 커밋에 섞지 않는다.** 롤백 난이도가 달라진다.
