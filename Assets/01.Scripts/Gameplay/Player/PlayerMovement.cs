...
namespace Game.Gameplay.Player
{
    ...
    public sealed class PlayerMovement : MonoBehaviour
    {
        ...
        {
            UpdateGroundedState();
            if (_hasGroundedState && IsGrounded && !_wasGrounded)
                _sfxChannel?.PlayOneShot(_landClip);
        ...
        {
            int hitCount = Physics2D.OverlapBox(_groundCheck.position, _stats.GroundCheckSize, 0f, _groundContactFilter, _groundHits);
            IsGrounded = false;
            for (int i = 0; i < hitCount; i++)
            {
                Collider2D groundHit = _groundHits[i];
                if (groundHit == null || groundHit.attachedRigidbody == _rigidbody) continue;
                IsGrounded = true;
                return;
            }
        }
